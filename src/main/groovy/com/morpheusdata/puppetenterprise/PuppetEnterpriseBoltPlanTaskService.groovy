...
@Slf4j
class PuppetEnterpriseBoltPlanTaskService extends AbstractTaskService {
	MorpheusContext context

	PuppetEnterpriseBoltPlanTaskService(MorpheusContext context) {
	...
	TaskResult executeLocalTask(Task task, Map opts, Container container, ComputeServer server, Instance instance) {
        ...
        String boltPlan = task.taskOptions.find { it.optionType.code == 'boltPlan' }?.value
        String boltPlanParameters = task.taskOptions.find { it.optionType.code == 'boltPlanParams' }?.value
		...
		def instanceMapping = morpheusContext.async.referenceData.find(new DataQuery().withFilters([
        ...
        taskPayload["plan_name"] = boltPlan
        taskPayload["params"] = [:]
        taskPayload["scope"] = [:]
        taskPayload["scope"]["nodes"] = ["${instanceMapping.keyValue}"]
        log.info "Plan PAYLOAD: ${planPayload}"
		HttpApiClient.RequestOptions requestOptions = new HttpApiClient.RequestOptions(headers:['X-Authentication':accessToken,'Content-Type':'application/json'],ignoreSSL: ignoreVerifySSL, body: planPayload)
		def account = new Account(id: config.accountId)

		def results = client.callJsonApi("${serviceUrl}:${orchestratorPort}", "orchestrator/v1/command/plan_run", "", "", requestOptions, 'POST')
        ...
        def jobOutputPayload = JsonOutput.toJson(jobresults.data.items[0].result).toString()
        new TaskResult(
            success: true,
            data   : "${jobOutputPayload}",
